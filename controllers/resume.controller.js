...
export const downloadResume = async (req, res) => {
  try {
    ...
    // 1. Fetch & validate resume record
    const resume = await Resume.findOne({ _id: id, userId });
    if (!resume || !resume.fileUrl) {
      console.warn("[downloadResume] Exit: Resume not found, unauthorized, or missing URL", { id, userId });
      return res.status(404).json({
        success: false,
        message: 'Resume not found, unauthorized, or has no file URL stored.',
      });
    }

    // 2. Fetch the file from Cloudinary storage
    const fileResponse = await fetch(resume.fileUrl);
    if (!fileResponse.ok) {
      console.error("[downloadResume] Storage fetch failed:", fileResponse.statusText);
      return res.status(fileResponse.status || 500).json({
        success: false,
        message: 'Failed to retrieve file from cloud storage.',
      });
    }

    // 3. Set headers and stream/download the file
